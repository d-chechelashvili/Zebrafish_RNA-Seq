...
2) **პირველ რიგში, გადმოვწერე sra-tools(v3.0.0) data-ს გადმოსაწერად.**

    2.1) კონდადან ვერ დავაყენე ამიტომ პირდაპირ გადმოვწერე საინსტალაციო.
    
    2.2) ყველაფრის ინსტრუქცია არის ამ ლინკზე https://github.com/ncbi/sra-tools/wiki/02.-Installing-SRA-Toolkit
3) **მოვძებნე data.**

    3.1) "The sequencing data have been deposited in NCBI Sequence Read Archive (SRA, http://www.ncbi.nlm.nih.gov/Traces/sra) and the accession number is SRA062881"
    
    3.2) NCBI-ს SRA ბაზაში მოვძებნე ეს accession number-ი.
    
    3.3) პირველივეზე გადავედი, იქიდან გადავედი შემდეგ all runs-ზე. მოვხვდი ამ ლინკზე. https://www.ncbi.nlm.nih.gov/Traces/study/?acc=SRP017563&o=acc_s%3Aa
    
    3.4) აქედან ამოვიღე run-ების კოდები. hypo არ ამოვიღე რადგანაც სხვა კვლევისაა.
4) **გადმოვწერე data. download.sh სკრიპტით.**

    4.1) რადგანაც layout-ში ვხედავთ რომ არის paired გადმოვწერთ ფაილებს სახელებით accession_number_1 და accession_number_2.
    
    4.2) ასევე "&" ოპერატორით თითოეული ფაილის გადმოწერა გაეშვება ბექგრაუნდში, ცალ-ცალკე პროცესებად, რაც საკმაოდ აასწრაფებს ყველაფერს.
    
5) **ამის შემდეგ უნდა დავიწყოთ RNA-seq workflow**

    5.1) თავიდან ვეცადე სტატიაში მოცემულ pipeline-ს გავყოლოდი მოცემული პროგრამების მოცემული ვერსიებით.
    
    5.2) უმეტესობა გამოყენებული პროგრამული tool-ების სტატიაში ნახსენები ვერსიები არ იშოვებოდა და ამიტომ ზოგი ჩავანაცვლე შედარებით ახალი ვერსიებით, რომლებიც იშოვებოდა.
    
    5.3) მაგრამ აღმოჩნდა, რომ მოცემული pipeline სიძველესთან ერთად დროისა და მეხსიერების მხრივ საკმაოდ დატვირთული იყო, რასაც ჩემი ლეპტოპი ვერ შესწვდებოდა.
    
    5.4) ამიტომ ვცადე მონაცემების დამუშავებისთვის ჩემით ამეწყო ახალი pipeline ახალი, შედარებით თანამედროვე პროგრამული tool-ების და ვერსიების გამოყენებით.
    
    5.5) ჯერ დავწერ ძველ pipeline-ზე სადამდე მივედი და როგორ გავყევი შემდეგ კი ახალ pipeline-ს.
...
2) **jupyter notebook-ისთვის მომიწია შემდეგი ბიბლიოთეკების დაინსტალირება**
...
3) **წყაროები და გამოსადეგი ბმულები:**
    ...
    3.2) aligner-ების შედარება: https://www.frontiersin.org/articles/10.3389/fpls.2021.657240/full  / https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7084517/
